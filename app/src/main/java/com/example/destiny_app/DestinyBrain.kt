...
class DestinyBrain {
    ...
    val currentStory: String
        get() = scenes[currentScene].title

    val currentOptions: Array<String>
        get() = arrayOf(
                scenes[currentScene].option1,
                scenes[currentScene].option2
        )

    @Throws(Exception::class)
    fun updateSceneForOption(number: Int) {
        val destination = scenes[currentScene].getDestinationForOption(number)
